...
df %>%
  ...
  geom_dl(data = df %>% filter(name == "Grichuk, Randal" | name == "Morales, Kendrys" | name == "Smoak, Justin"), aes(label = name, colour = "Blue Jays"),  method = list(dl.combine("first.points", "last.points"), cex = 0.8)) +
scale_colour_brewer(palette = "Spectral") +
  theme_bw() +
  
  labs(x= "Season", y = "Hit% with EV95+",
       colour = NULL,
       title = "HIT% WITH EV95+ BY SEASON",
       subtitle = "Hard hit balls (95+ mph) from MLB Players with minimum 25 AB",
       caption = "@datajake | Source: baseballsavant.com") +
  theme_bw() +
  theme(panel.border = element_blank(),
        text = element_text(colour = "gray60", family = "wqy-microhei"),
